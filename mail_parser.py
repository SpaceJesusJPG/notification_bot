...
async def mail_receiver(user, password, imap_server):
...
async def get_most_recent_readings(kedr_messages):
    result = {}
    for facility, msg_lst in kedr_messages.items():
        voltages = await parse_message_list(msg_lst)
        result.setdefault(FACILITIES[facility], []).extend(voltages)
    return result


async def lowest_reading(most_recent_readings):
    result = []
